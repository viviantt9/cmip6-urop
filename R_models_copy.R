...
df <- np$load("global_mean_tas.npy") [,2]
...
data <- ts(df, start = c(1900, 1), frequency = 1)
train_data <- ts(df, start = c(1900, 1), frequency = h)
...
# ggseasonplot(train_data, year.labels = TRUE, year.labels.left = TRUE) +
...
# print(paste("Average MAE across all the horizons for model",
#   model_name, ":", round(mean(mae), 1)))

# # store MAE
# models_mae <- rbind(models_mae,
#   data.frame(model = model_name, mae,horizon = 1:h,row.names = NULL))
...
# model <- function(y, h) {
#   stl_decomposition <- stl(y, t.window = 13,
...
####### SARIMA
print(auto.arima(train_data, approximation = FALSE, seasonal = F))
model_name <- "SARIMA"
model <- function(y, h) {
  forecast(Arima(y, order = c(0, 1, 2), include.drift = T), h = 12)
}

print(1)

plot1 = my_forecast_plot(model, model_name)

# compute CV MAE
cv_residuals <- tsCV(train_data, model, initial = 0, h = h)
mae <- mean(cv_residuals, na.rm = T)
print(paste("Average MAE across all the horizons for model",
  model_name, ":", mae, 1))
...
# ####### ARIMA + Decomposition
# stl_decomposition <- stl(train_data, t.window = 13,
#   s.window = "periodic", robust = TRUE)
# print(auto.arima(seasadj(stl_decomposition),
#   stepwise = FALSE, approximation = FALSE))

# model_name <- "ARIMA + Decomposition"
# model <- function(y, h) {
#   fit <- stlm(y, s.window = "periodic", robust = TRUE,
#     modelfunction = Arima, order = c(3, 1, 1))
...
# # compute CV MAE
# cv_residuals <- tsCV(train_data,model, initial = 0, h = h)
# mae <- mean(abs(cv_residuals), na.rm = T)
# print(paste("Average MAE across all the horizons for model",
#   model_name, ":", round(mean(mae), 1)))
...
#     ugarchfit(diff_y_adjusted, solver = 'hybrid',
...
#   last_year_seasonality <- window(seasonal(stl_decomposition),
#     start = new_start)

#   # sum them up
#   fcast <- output_garch +
#     ts(last_year_seasonality, frequency = h, start = start(output_garch))
...
#     m0 = c(x[5], x[6]), C0 = exp(x[7]) * diag(nrow = 2)) 
#   return(m)
# }
...
#   # sum them up
#   fcast <- output +
#     ts(last_year_seasonality, frequency = h, start = start(output))
...
# model_name <- "TBATS"
# model <- function(y, h) {
#   return(forecast(tbats(y, use.box.cox = TRUE,
#     use.damped.trend = F, seasonal.periods = c(12),
#     start.p = 2, max.p = 2, start.q = 1, max.q = 1), h = h))
# }
...
#     weekly.seasonality = F, daily.seasonality = F) 
#   # make predictions
#   future <- make_future_dataframe(m,
#     freq = "month", include_history = False, periods = h)
...
# model <- function(y, h) {
#   fit <- stlm(y, s.window = "periodic", # nolint
#     robust = TRUE, modelfunction = nnetar, p = 2, P = 0)
#   return(forecast(fit, h = h)) # nolint
# }
# my_forecast_plot(model, model_name)
...
# plot_scores(models_mae, naive = False)
# plot_scores(models_mae[models_mae$model %in%
#   c("SARIMA", "TBATS", "RW+Decomposition", "NNETAR + Decomposition"), ])
...
# fitted_nnetar <- stlm(train_data, s.window = "periodic",
#   robust = TRUE, modelfunction = nnetar, p = 2, P = 0)
# test_residuals <- colMeans(
#   abs(evaluate_test_residuals(fitted_nnetar, stlm, data)), na.rm = T)
# print(paste("Mean abs. error NNETAR+Decomposition on test set:",
#   round(mean(test_residuals), 2)))
